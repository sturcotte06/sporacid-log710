...
void main (void) {    
    ...
    while (true) {
        ...
        if (strcmp(argv[0], "exit") == 0) {
            ...
            if (isBackground) {                
                // Run asynchronously.
                runBackgroundTask(argc + 1, execArgv);
            } else {
                // Run synchronously.
                runTask(argc + 1, execArgv);
            }
        }
...
int changeDirectory(const char* path) {
    char* newDirectory;
    int currentDirectoryLen, pathLen = strlen(path);
    
    // If the first char is a dot, special case.
    if (path[0] != '/') {
        printf("no slash\n");
        combinePath(currentDirectory, currentDirectoryLen, path, pathLen);
    } else {
...
void runBackgroundTask(int argc, char** argv) {
    ...
    if (pid == 0) {
        ...
        // // We do not want async tasks to output in the shell while it runs in background.
    ...
    char* taskName = calloc(strlen(argv[1]), sizeof(char));
    ...
    int id = task.id;
    pthread_t thread;
    pthread_create(&thread, NULL, switchFinishedFlag, &id);
