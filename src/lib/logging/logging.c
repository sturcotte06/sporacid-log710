...
void get_formatted_message(char* fmessage, const unsigned int level, const char *file, int line, const char *func, const char* format, va_list args);
...
const char* LEVEL_COLOURS[LEVEL_COUNT] = { ANSI_COLOR_RESET, ANSI_COLOR_RESET, ANSI_COLOR_RESET, ANSI_COLOR_YELLOW, ANSI_COLOR_RED, ANSI_COLOR_RED };
...
void logging_format_tofile(FILE* stream, const unsigned int level, const char *file, int line, const char *func, const char* format, ...) {
	...
	get_formatted_message(fmessage, level, file, line, func, format, args);

    // Print the message in the stream.
    fputs(fmessage, stream);

    // Unload variable args list.
    va_end(args);
}
...
void get_formatted_message(char* fmessage, const unsigned int level, const char *file, int line, const char *func, const char* format, va_list args) {    
    ...
    int len;
    len = sprintf(fmessage, " %s%-7s | %s | %-50.50s | %-10u | ",
